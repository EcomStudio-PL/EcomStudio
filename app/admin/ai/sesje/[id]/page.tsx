...
export default async function AdminConceptSessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/admin/ai/prompts?tab=history" className="mb-3 inline-flex items-center gap-1.5 text-sm text-muted hover:text-ink">
          ...
          return (
            <Panel key={p.id} className="rounded-2xl p-4 sm:p-5">
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                {p.scene_type && <Badge tone="accent">{p.scene_type}</Badge>}
                <Badge>{p.lock_strength}</Badge>
                {payload?.meta.tv != null && <Badge tone="success">tpl v{payload.meta.tv}</Badge>}
