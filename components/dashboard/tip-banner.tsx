...
export function TipBanner({ id, text, ctaLabel, ctaHref }: {
  ...
  return (
    ...
    <div className="animate-rise relative flex flex-col gap-3 overflow-hidden rounded-2xl px-4 py-2.5 sm:flex-row sm:items-center sm:gap-4 sm:px-4
      ...
      <div className="relative flex min-w-0 flex-1 items-start gap-3">
        <span aria-hidden className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-white/20 text-white ring-1 ring-white/25">
          <Lightbulb size={15} />
        </span>
        <p className="min-w-0 text-[12.5px] leading-snug text-white">
          <span className="font-semibold">{t("dashboard.tipTitle")}</span>{" "}
