...
import { CATEGORIES, offeredWorkflows, type Category } from "./categories";
import { TOOL_CARDS, type ToolCardDef } from "./tool-cards";
import {
...
export function homeSections(avail: AvailabilityMap, isAdmin = false): HomeSection[] {
  ...
  if (menuVisible(avail, "/wideo", isAdmin)) {
    const cards = pick(TOOL_CARDS.filter((c) => c.key.startsWith("video_")).map((c) => c.key));
    sections.push({
      key: "video",
