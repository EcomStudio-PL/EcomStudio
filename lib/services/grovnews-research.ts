...
import type { EditionStatus, GrovNewsSettings, ItemStatus, SourceType } from "@/lib/grovnews-research";
import { DEFAULT_SETTINGS } from "@/lib/grovnews-research";
...
export async function adminListSources(supabase: Client): Promise<AdminSource[]> {
  const { data } = await supabase.from("grovnews_sources")
    .select("id, name, source_type, url, enabled, category_id, priority, official_source, language, last_checked_at, last_success_at, last_error, category:grovnews_categories(name)")
    .order("enabled", { ascending: false }).order("priority", { ascending: false }).order("name").limit(500);
  type Row = {
